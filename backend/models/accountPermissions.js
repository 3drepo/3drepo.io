...
const { listProjects, updateAttrs } = require("./project");
...
AccountPermissions.remove = async function(teamspace, userToRemove) {
	...
	const projects = await listProjects(teamspace.user, { "permissions.user": userToRemove});
	await Promise.all(
		projects.map(proj => updateAttrs(teamspace.user, proj.name, {
