...
schema.statics.getBCFZipReadStream = function(account, model, username, branch, revId){
	...
	return ModelSetting.findById({account, model}, model).then(_settings => {
		...
		issues.forEach(issue => {
			...
			promises.push(issue.getBCFMarkup(_.get(settings, "properties.unit"), account, model, username, branch, revId)
				.then(bcf => {
					zip.append(new Buffer(bcf.markup, "utf8"), {name: `${uuidToString(issue._id)}/markup.bcf`});
					systemLogger.logError(`bcf.markup ${bcf.markup}`);
					...
					bcf.viewpoints.forEach(vp => {
						systemLogger.logError(`vp.xml ${vp.xml}`);
...
schema.methods.getBCFMarkup = function(unit, account, model, username, branch, revId){
	...
	if (_.get(this, "due_date")) {
		markup.Markup.Topic.DueDate = _.get(this, "due_date");
	...
	return this.getIfcIdMaps(account, model, username, branch, revId).then(ifcIdMaps => {
		...
		this.viewpoints.forEach((vp, index) => {
			...
			/*ModelHelper.getAllIdsWith4DSequenceTag(account, model, branch, revId).then(function (response) {
				systemLogger.logError(response.data.length);
				viewpointXmlObj.fourdids = response;
			});*/
			/*ModelHelper.getAllIdsWithMetadataField(account, model, branch, revId, "IFC GUID", username)
				.then(function (metadataResponse) {
					for (let i = 0; i < metadataResponse.data.length; i++) {
						systemLogger.logError(`${metadataResponse.data[i].metadata.value}`);
					}
					viewpointXmlObj.ifcguids;
				});;*/
			//systemLogger.logError(`ifcGuidToId ${ifcIdMaps.ifcGuidToId['00XuLiRb9DCxjpnLyB07o8']}`);
			...
			if (_.get(vp, "extras.ClippingPlanes")) {
			...
			function createIssue(guid){
				...
				return parseXmlString(markupBuf.toString("utf8"), {explicitCharkey: 1, attrkey: "@"}).then(_xml => {
					...
					if(xml.Markup){
						...
						issue.due_date = _.get(xml, "Markup.Topic[0].DueDate[0]._");
