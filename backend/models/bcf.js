...
function processMarkup(markup) {
	...
	if (markup.Markup) {
		issue.extras.Header = _.get(markup, "Markup.Header");
		if (_.get(markup, "Markup.Topic[0].@.TopicType")) {
			issue.topic_type = _.get(markup, "Markup.Topic[0].@.TopicType");
		}
		issue.status = sanitise(_.get(markup, "Markup.Topic[0].@.TopicStatus"), statusEnum);
		if (!issue.status || issue.status === "") {
			issue.status = "open";
		}
		issue.extras.ReferenceLink = _.get(markup, "Topic[0].ReferenceLink");
		issue.name = _.get(markup, "Markup.Topic[0].Title[0]._");
		issue.priority = sanitise(_.get(markup, "Markup.Topic[0].Priority[0]._"), priorityEnum);
		issue.extras.Index = _.get(markup, "Markup.Topic[0].Index[0]._");
		issue.extras.Labels = _.get(markup, "Markup.Topic[0].Labels[0]._");
		issue.created = moment(_.get(markup, "Markup.Topic[0].CreationDate[0]._")).format("x").valueOf();
		issue.owner = _.get(markup, "Markup.Topic[0].CreationAuthor[0]._");
		issue.extras.ModifiedDate = _.get(markup, "Markup.Topic[0].ModifiedDate[0]._");
		issue.extras.ModifiedAuthor = _.get(markup, "Markup.Topic[0].ModifiedAuthor[0]._");
		if (_.get(markup, "Markup.Topic[0].DueDate[0]._")) {
			issue.due_date = moment(_.get(markup, "Markup.Topic[0].DueDate[0]._")).valueOf();
		}
		if (_.get(markup, "Markup.Topic[0].AssignedTo[0]._")) {
			issue.assigned_roles = _.get(markup, "Markup.Topic[0].AssignedTo[0]._").split(",");
		} else {
			issue.assigned_roles = [];
		}

		issue.desc = (_.get(markup, "Markup.Topic[0].Description[0]._")) ? _.get(markup, "Markup.Topic[0].Description[0]._") : "(No Description)";
		issue.extras.BimSnippet = _.get(markup, "Markup.Topic[0].BimSnippet");
		issue.extras.DocumentReference = _.get(markup, "Markup.Topic[0].DocumentReference");
		issue.extras.RelatedTopic = _.get(markup, "Markup.Topic[0].RelatedTopic");
	}

	_.get(markup, "Markup.Comment") && markup.Markup.Comment.forEach(comment => {
		const obj = {
	...
	return {
		issue,
		viewpointsData: markup.Markup.Viewpoints
	};
}
...
function readBCF(account, model, requester, ifcToModelMap, zipPath, settings) {
	return new Promise((resolve, reject) => {
		...
		yauzl.open(zipPath, {lazyEntries: true}, function (zipErr, zipFile) {
			if (zipErr) {
			...
			zipFile.on("entry", entry => {
				...
				if (!entry.fileName.endsWith("/") && !entry.fileName.endsWith("\\") && guid) {
					readPromises.push(new Promise((_resolve, _reject) => {
			...
			zipFile.on("end", async () => {
				...
				Object.keys(files).forEach(guid => {
					...
					Object.keys(files[guid]).forEach((fileName) => {
						const isXmlFile = fileName.match(/.*\.(bcf|bcfv)$/i);

						if (isXmlFile && files[guid][fileName]) {
							const parsedResult = parseXmlString(files[guid][fileName].toString("utf8"), { explicitCharkey: 1, attrkey: "@" });
							parsePromises.push(parsedResult);
							parsedFiles[guid][fileName] = parsedResult;
						} else {
							parsedFiles[guid][fileName] = files[guid][fileName];
						}
					});
				});

				console.log(parsePromises);
				...
				const end = await Promise.all(parsePromises);
...
class BCF {
	async getReadStream(account, model, issues, unit) {
	...
	async importFile(requester, account, model, zipPath, settings) {
