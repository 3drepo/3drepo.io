...
const { getIdToMeshes } = require("./jsonAssets");
...
async function findModelSharedIdsByRulesQueries(account, model, posRuleQueries, negRuleQueries, branch, revId, username, convertSharedIDsToString) {
	...
	const idToMeshesDict = await getIdToMeshes(account, model, branch, revId, username);
...
Meta.getAllIdsWithMetadataField = function(account, model, branch, rev, fieldName, username) {
