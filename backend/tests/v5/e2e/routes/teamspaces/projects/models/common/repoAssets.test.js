...
const createContRevisionObject = (isVoid, hasFile, date, isRepoBundle) => {
	const rev = ServiceHelper.generateRevisionEntry(isVoid, hasFile, modelTypes.CONTAINER, date);
	rev.isRepoBundle = isRepoBundle;
	return rev;
};
...
const generateTestEnvData = () => {
	// - Container C1:
	// - Valid revision R1 RepoBundle
	// - Valid revision R2 AssetBundle
	// - Container C2:
	// - Valid Revision R3 RepoBundle
	// - Container C3:
	// - Void revision R4
	// - Valid Revision R5 RepoBundle
	// - Container C4:
	// - NoFile revision R6
	// - Valid Revision R7 RepoBundle
	// - Container C5:
	// - Void Revision R8
	// - Container C6:
	// - Void Revision R9
	...
	const R1 = createContRevisionObject(false, true, dates[0], true);
	const R2 = createContRevisionObject(false, true, dates[1], false);

	// Create Revisions for C2
	const R3 = createContRevisionObject(false, true, dates[2], true);

	// Create Revisions for C3
	const R4 = createContRevisionObject(true, true, dates[3], undefined);
	const R5 = createContRevisionObject(false, true, dates[4], true);

	// Create Revisions for C4
	const R6 = createContRevisionObject(false, false, dates[5], undefined);
	const R7 = createContRevisionObject(false, true, dates[6], true);

	// Create Revisions for C5
	const R8 = createContRevisionObject(true, true, dates[7], undefined);

	// Create Revisions for C6
	const R9 = createContRevisionObject(true, true, dates[8], undefined);
...
const testGetRepoAssets = () => {
	describe('Get repo assets', () => {
		...
		const generateTestData = () => {
			...
			const getContResult = (cont, revId) => {
				...
				if (revisionId) {
					...
					if (stashEntry) {
						// Treat fields that were added for easier handling in mocking but would
						// not be present in the real thing.
						const formattedStashEntry = { ...stashEntry };
						delete formattedStashEntry.jsonData;
						delete formattedStashEntry.repobundleStash;
						formattedStashEntry._id = uuidHelper.stringToUUID(formattedStashEntry._id);
						models.push(formattedStashEntry);
					}
				}
			...
			const getFedResult = (fed) => {
				...
				for (let i = 0; i < conts.length; i++) {
					...
					if (revisionId) {
						const stashEntry = cont.stashEntries.find((entry) => entry._id === revisionId);
			...
			const contValidRevs = [
				...
				['trying to access current without supplying a revision', getContRoute({ revisionId: null }), true, curRevResult],
				...
				['trying to access current with viewer without supplying a revision', getContRoute({ revisionId: null, key: viewerKey }), true, curRevResult],
				['trying to access current with commenter without supplying a revision', getContRoute({ revisionId: null, key: commenterKey }), true, curRevResult],
			];
			...
			const contVoidRevs = [
				...
				['trying to get latest from container with newer void revision without supplying a revision (admin)', getContRoute({ modelId: C3Id, revisionId: null }), true, validRevResultC3],
				['trying to get latest from container with newer void revision without supplying a revision (viewer)', getContRoute({ modelId: C3Id, revisionId: null, key: viewerKey }), true, validRevResultC3],
				['trying to get latest from container with newer void revision without supplying a revision (commenter)', getContRoute({ modelId: C3Id, revisionId: null, key: commenterKey }), true, validRevResultC3],
			];
			...
			const contNoFileRevisions = [
				...
				['trying to access noFile revision via without supplying a revision (admin)', getContRoute({ modelId: C4Id, revisionId: null }), true, noFileRevResult],
				['trying to access noFile revision via without supplying a revision (viewer)', getContRoute({ modelId: C4Id, revisionId: null, key: viewerKey }), true, noFileRevResult],
				['trying to access noFile revision via without supplying a revision (commenter)', getContRoute({ modelId: C4Id, revisionId: null, key: commenterKey }), true, noFileRevResult],
			];
		...
		describe.each(generateTestData())('Get repo assets', runTest);
	});
};
