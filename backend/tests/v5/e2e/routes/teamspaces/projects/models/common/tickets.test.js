...
const testUpdateComment = () => {
	describe('Update comment', () => {
		...
		const runTest = (desc, { model, ...routeParams }, success, expectedOutput) => {
			...
			test(`should ${success ? 'succeed' : `fail with ${expectedOutput.code}`} if ${desc}`, async () => {
				const endpoint = putRoute({ modelId: model._id,
					ticketId: model.ticket?._id,
					commentId: model.comment?._id,
					...routeParams });
				const expectedStatus = success ? templates.ok.status : expectedOutput.status;
				...
				if (success) {
					...
					const expectedComment = {
						...
						history: [{ comment: model.comment.comment, timestamp: updatedComment.updatedAt }],
					};
			...
			test(`should ${success ? 'succeed' : `fail with ${expectedOutput.code}`} if ${desc}`, async () => {
				...
				if (success) {
					...
					const expectedComment = {
						...
						history: [{ comment: model.comment.comment, timestamp: deletedComment.updatedAt }],
					};
