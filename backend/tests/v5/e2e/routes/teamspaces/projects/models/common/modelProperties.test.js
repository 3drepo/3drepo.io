...
const ModelProperties = require(`${src}/routes/teamspaces/projects/models/common/modelProperties`);
	...
	describe('Get model properties', () => {
		...
		const generateTestData = () => {
			const getContRoute = ({
				...
				let route;
				if (revisionId) {
					route = `/v5/teamspaces/${ts}/projects/${projectId}/${modelType}s/${modelId}/assets/properties?revision=${revisionId}&key=${key}`;
				} else {
					route = `/v5/teamspaces/${ts}/projects/${projectId}/${modelType}s/${modelId}/assets/properties?key=${key}`;
				}
				return route;
			};
			...
			const contNoFileRevisions = [
				...
				['trying to access noFile revision without supplying a revision', getContRoute({ modelId: C4Id, revisionId: null }), true, noFileResult],
			];
...
const testEstablishRoutes = () => {
	describe('Create routes for asset maps', () => {
		...
		test('Should create one route for Federations', () => {
			const router = ModelProperties(modelTypes.FEDERATION);
		...
		test('Should create no routes for Drawings', () => {
			const router = ModelProperties(modelTypes.DRAWING);
