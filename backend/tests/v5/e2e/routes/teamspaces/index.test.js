...
const { createAccount } = require('../../../helper/fronteggMock/components/accounts');
...
const testGetTeamspaceList = (isInternal) => {
	describe('Get teamspace list', () => {
		const route = (key) => `/v5/teamspaces/${key ? `?key=${key}` : ''}`;
		if (isInternal) {
			test('should fail with a page not found error', async () => {
				const res = await agent.get(route()).expect(templates.pageNotFound.status);
				expect(res.body.code).toEqual(templates.pageNotFound.code);
			});
		}
		if (!isInternal) {
			...
			test('return a teamspace list if the user has a valid session', async () => {
				const res = await agent.get(route(testUser.apiKey)).expect(templates.ok.status);
				const expectedList = [
...
const testGetTeamspaceMembers = (isInternal) => {
...
const testGetAvatar = (isInternal) => {
...
const testGetQuotaInfo = (isInternal) => {
	describe('Get quota info', () => {
		...
		if (isInternal) {
			...
			const spaceLimitBasicDiscretionary = config.subscriptions?.basic?.data + activeLicense.discretionary.data;
			const spaceLimitInBytes = spaceLimitBasicDiscretionary * 1024 * 1024;
			const expectedQuota = {
...
const testRemoveTeamspaceMember = (isInternal) => {
	describe('Remove teamspace member', () => {
		...
		if (isInternal) {
			test('should fail with a page not found error', async () => {
				const res = await agent.delete(route()).expect(templates.pageNotFound.status);
				expect(res.body.code).toEqual(templates.pageNotFound.code);
			});
		} else if (!isInternal) {
			beforeAll(async () => {
...
const testGetMemberAvatar = (isInternal) => {
...
const testGetAddOns = (isInternal) => {
