...
queue.purgeQueues = async () => {
	...
	await Promise.all([
		purgeQueue(worker_queue).then(() => console.log('worker queue done')),
		purgeQueue(model_queue).then(() => console.log('model queue done')),
		purgeQueue(callback_queue).then(() => console.log('callback queue done')),
	]);
