...
const SystemTemplate = require(`${src}/services/mailer/templates/systemTemplate`);

const testHtml = () => {
	describe('get data template html', () => {
		describe.each([
			...
			'Error checking ', (desc, data) => {
				test(`should throw an error if ${desc}`, async () => {
					await expect(SystemTemplate.html(data)).rejects.toThrow();
				});
			},
		);
		...
		test('should get base template html', async () => {
			...
			const res = await SystemTemplate.html(data);
