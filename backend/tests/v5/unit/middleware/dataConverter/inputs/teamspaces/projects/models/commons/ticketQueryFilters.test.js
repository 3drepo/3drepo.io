...
const testValidateQueryString = () => {
	...
	const stringValueTests = [queryOperators.IS, queryOperators.NOT_IS, queryOperators.CONTAINS,
		...
		.flatMap((operator) => ([
			...
			[`operator is ${operator} and a value contains double quotes`, getQueryProp(propName, operator, '"a,happy,person","%22happy%22%20is%20fake","%22%22"'), true, [{ operator, propertyName: `properties.${propName}`, value: ['a,happy,person', '"happy" is fake', '""'] }]],
