...
describe('Chat service', () => {
	...
	const username = 'collaboratorTeamspace1Model1RoleA';
	...
	const updateIssue = (agent, id, partialIssue) => (next) => agent.patch(`/${account}/${model}/issues/${id}`)
		.send(partialIssue)
		.expect(200, (err, res) => {
			if (next) next(err);
		});
	...
	before((done) => {
		...
		server = createApp().listen(8080, () => {
			...
			chatServer.listen(config.chat_server.port, () => {
				async.series([
					function (done) {
	...
	it('connect to chat server and join room should succeed', function (done) {
		...
		socket.on('connect', (data) => {
			...
			socket.on('credentialError', (err) => {
				done(err);
			});
		});
	});

	it('join a room that user has no access to should fail', (done) => {
		// newXhr.setCookies(`connect.sid=${connectSid}; `);

		// https://github.com/socketio/socket.io-client/issues/318 force new connection
		const mySocket = io(config.chat_server.chat_host, { path: `/${config.chat_server.subdirectory}`,
		...
		mySocket.on('connect', (data) => {
			...
			mySocket.on('credentialError', (err) => {
				expect(err).to.exist;
				done();
			});
		});
	...
	it('subscribe new issue chat event should succeed', (done) => {
		// other users post an issue
		const issue = { name: 'Issue test', ...baseIssue };

		socket.on(`${account}::${model}::issueCreated`, (issues) => {
	...
	it('subscribe new comment chat event should succeed', (done) => {
		...
		socket.on(`${account}::${model}::${issueId}::commentCreated`, (resComment) => {
...
*/
	...
	it('subscribe comment deleted chat event should succeed', (done) => {
		const comment = { guid: commentGuid };

		socket.on(`${account}::${model}::${issueId}::commentDeleted`, (resComment) => {
			expect(resComment).to.exist;
			done();
		});
	...
	it('subscribe issue change should succeed', (done) => {
		...
		async.parallel([
			...
			function (done) {
				socket.on(`${account}::${model}::issueUpdated`, (issue) => {
	...
	describe('with notifications', () => {
		...
		describe('of issue assigned type', () => {
			...
			it('should receive a new notification event when a issue has been created',
				testForChatEvent(() => {
					createIssue(teamSpace1Agent, roleAIssue)((err, iss) => issueId = iss._id);
				},
			...
			it('should receive an upsert event when another issue of the same model has been assigned to the user',
				testForChatEvent(() => {
					createIssue(teamSpace1Agent, roleAIssue)((err, iss) => issueId2 = iss._id);
				},
				notificationUpsertEvent,
				(notifications, done) => {
		...
		describe('of closed issue type', () => {
			before((done) => async.waterfall([
				...
				createIssue(teamSpace1Agent, roleAIssue),
