...
const { createFederation, createModel, updateFederation } = require('../helpers/models');
...
describe('Notifications', function () {
	...
	const usernames = ['unassignedTeamspace1UserRoleA',
		'viewerTeamspace1Model1RoleA',
		'viewerTeamspace1Model1RoleB',
		'commenterTeamspace1Model1RoleA',
		'commenterTeamspace1Model1RoleB',
		'collaboratorTeamspace1Model1RoleA',
		'collaboratorTeamspace1Model1RoleB',
		'adminTeamspace1RoleA',
		'adminTeamspace1RoleB',
		'viewerTeamspace1Model1RoleC',
		'collaboratorTeamspace1Model1RoleC',
		'viewerTeamspace1Model1RoleD',
	...
	const updateIssue = (modelId, issue, id) => (next) => agents.teamSpace1.patch(`/${account}/${modelId}/issues/${id}`)
		.send(issue)
		.expect(200, (err, res) => {
			next(err);
		});
	...
	const deleteAllNotifications = (next) => async.parallel(
	...
	// ========================================
	describe('of type assign issue', () => {
		const issuesId = [];

		const assignIssue = (modelId, name) => (done) => {
			const issue = { name: (name || 'Assign notification test'), ...baseIssue };
			const issueAssignRoleA = { ...issue, assigned_roles: ['roleA'] };
			async.series([(next) => agents.teamSpace1.post(`/${account}/${modelId}/issues`)
				.send(issue)
				.expect(200, (err, res) => {
			...
			(next) =>	updateIssue(modelId, issueAssignRoleA, issuesId[issuesId.length - 1])(next),
			], done);
		};

		before(assignIssue(model));

		it('should not be created for viewers and roleB type users', (done) => {
			const users = ['unassignedTeamspace1UserRoleA',
				'viewerTeamspace1Model1RoleA',
				'viewerTeamspace1Model1RoleB',
				'commenterTeamspace1Model1RoleB',
				'collaboratorTeamspace1Model1RoleB',
		...
		it('should be created for commenters/collaboratos of roleA type users (just one notification) ', (done) => {
			const users = ['commenterTeamspace1Model1RoleA',
				'collaboratorTeamspace1Model1RoleA',
				'adminTeamspace1RoleA'];
		...
		it('should not be able to be deleted by other people which are not the user that were adressed originaly', (done) => {
			async.waterfall([
				fetchNotification(agents.collaboratorTeamspace1Model1RoleA),

				(notifications, next) => agents.commenterTeamspace1Model1RoleA
		...
		it('should be able to be deleted by the user which was addressed originally', (done) => {
			async.waterfall([
				fetchNotification(agents.collaboratorTeamspace1Model1RoleA),
				(notifications, next) => agents.collaboratorTeamspace1Model1RoleA
					.delete(`${NOTIFICATIONS_URL}/${notifications[0]._id}`)
					.expect(200, next),
		...
		it('should be updated if a another issue has been assign in the same model', (done) => {
			async.waterfall([
				...
				fetchNotification(agents.adminTeamspace1RoleA),
				(notifications, next) => {
					expect(notifications).to.be.an('array').and.to.have.length(1);
		...
		it('should remove an issue Id if the issue has been assign to a diferent profile', (done) => {
			const issue = { ...baseIssue, assigned_roles: [] };
			const issueRoleA = { ...issue, assigned_roles: ['roleA'], name: 'back to roleA' };
			...
			async.waterfall([
				...
				updateIssue(model, issueRoleA, issueID1),
				fetchNotification(agents.adminTeamspace1RoleA),
				(notifications, next) => {
					expect(notifications).to.be.an('array').and.to.have.length(1);
		...
		it('should remove an issue id if the issue has been closed', (done) => {
			const issue = { ...baseIssue, status: 'closed' };
		...
		it('should be created when a new issue has being created with a RoleA assigned directly', (done) => {
			const issue = { name: 'New notification for roleA ', ...baseIssue };
			const issueRoleA = { ...issue, assigned_roles: ['roleA'] };

			async.waterfall([
				(next) => {
					agents.adminTeamspace1RoleA.delete(NOTIFICATIONS_URL)
						.expect(200, () => {
							next();
						});
				},
	...
	// ========================================
	describe('of type closed issue', () => {
		const issuesId = [];
		const issue = { ...baseIssue, name: 'Closed notification test', assigned_roles: ['roleB'] };
		...
		const closeIssue = (model, id) => (done) => updateIssue(model, Object.assign(issue, { status: 'closed' }), id)(done);

		const uncloseIssue = (model, id) => (done) => updateIssue(model, Object.assign(issue, { status: 'in progress' }), id)(done);
		...
		it("shouldn't have a closed issue notification",
			testForNoClosedNotification('adminTeamspace1RoleA'),
		);
		...
		describe('when a issue has been closed', () => {
			...
			it("the collaborator of owner's role should get a closed notification",
				testForIssue0('collaboratorTeamspace1Model1RoleA'),
			);

			it("the commenter of owner's role should get a closed notification",
				testForIssue0('commenterTeamspace1Model1RoleA'),
			);

			it("the viewer of owner's role should not get a closed notification",
				testForNoClosedNotification('viewerTeamspace1Model1RoleA'),
			);
		});
		...
		describe('when two issues has been closed', () => {
			...
			it('collaborator from first assigned role in issue should have a closed issue notification wth two issues ids',
				testForBothIssues('collaboratorTeamspace1Model1RoleB'),
			);
	...
	describe('of type model update', () => {
		...
		const usersWithPermission = ['viewerTeamspace1Model1RoleA',
			'viewerTeamspace1Model1RoleB',
			'commenterTeamspace1Model1RoleA',
			'commenterTeamspace1Model1RoleB',
			'collaboratorTeamspace1Model1RoleA',
		...
		it('should be not be created for users without access to the model', (done) => {
			agents.unassignedTeamspace1UserRoleA.get(NOTIFICATIONS_URL)
				.expect(200, (err, res) => {
