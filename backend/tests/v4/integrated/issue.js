...
describe('Issues', () => {
	...
	const commenterUser = 'commenterTeamspace1Model1RoleA';
	...
	describe('Creating an issue', () => {
		...
		it('change assigned_roles should succeed and create system comment', (done) => {
			const issue = { name: 'Issue test', ...baseIssue, assigned_roles: ['roleA'] };
			let issueId;
			const data = { assigned_roles: ['roleB'] };
			async.series([
				function (done) {
					agent.post(`/${username}/${model}/issues`)
						.send(issue)
		...
		describe('user who is collaborator/commentor and assigned to the issue role can', () => {
			const issue = { name: 'Issue test', ...baseIssue };
			let issueId;
		...
		describe('user who is collaborator/commentor but not assigned to issue role can', () => {
			let issueId;

			before((done) => {
				const issue = Object.assign(baseIssue, { name: 'Issue test', assigned_roles: ['roleC'] });
		...
		describe('user with different role but is an admin', () => {
			const issue = { ...baseIssue, name: 'Issue test', creator_role: 'roleC' };
			let issueId1;
			let issueId2;
	...
	describe('Search', () => {
		...
		before((done) => {
			...
			adminAgent.login('adminTeamspace1RoleA', password).then(() => {
				async.series([
					(next) => {
