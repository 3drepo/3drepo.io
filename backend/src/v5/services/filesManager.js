...
const getFileAsStream = async (teamspace, collection, refEntry, chunkInfo) => {
	const { type, link, size, mimeType, encoding, name: filename } = refEntry;
	let readStream;
	...
	return { readStream, size, mimeType: mimeType ?? DEFAULT_MIME_TYPE, encoding, filename };
};
...
FilesManager.getMD5FileHash = async (teamspace, container, filename) => {
	...
	const file = await FilesManager.getFile(teamspace, `${container}.history`, filename);
