...
TeamspacePerms.hasMemberAccessToTeamspace = async (req, res, next) => {
	const { params } = req;
	...
	if (hasAccess) {
		...
		respond(req, res, createResponseCode(templates.notAuthorized, 'Member does not have access to the teamspace'));
	}
};
