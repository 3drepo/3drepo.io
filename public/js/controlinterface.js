...
(function() {
	...
	window.startPoint = [-26.06, 0, 15.28];
	//window.endPoint   = [-4.06, 1.43, 15.28];
	window.endPoint   = [-26.06, 0, -7.28];
	window.scaleY     = 0.05;
	window.trans	  = 0.3;
...
function sendRecording()
{
	...
	$.ajax({
		type: "POST",
		url:  "//api.example.org:8081/wayfinder/record",
...
function addSphere(position, radius, scaleY, rgb, trans)
{
	...
	var newnode = "<transform scale='" + scaleStr + "'translation='" + posStr + "'><shape isPickable='false'><appearance><material diffuseColor='" + rgbStr + "' transparency='" + trans + "'></material></appearance><sphere radius='" + radius + "' solid=false></sphere></shape></tranform>";
