...
const printEmptyRows = (ElasticClient, ts, licenseType, currentM, currentY, month, year) => {
	if(currentM !== -1) {
		...
		while(!(nextM === month && nextY === year)) {
			...
			Utils.createElasticRecord(ElasticClient, Utils.teamspaceIndexPrefix + ts, ts, elasticBody )
...
TS.createTeamspaceActivityReport = (dbConn, ElasticClient, ts, licenseType) =>{
	return new Promise((resolve, reject) => {
		// const fname = Utils.generateFileName(`${elastic}/activity/${ts}_activity`);
