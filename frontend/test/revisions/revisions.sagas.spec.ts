...
describe('Revisions: sagas', () => {
	...
	const mockRevision = revisionsMockFactory({ _id: revisionId, timestamp: null });
	...
	describe('setVoidStatus', () => {
		...
		beforeEach(() => {
			dispatch(RevisionsActions.fetchSuccess(containerId, [voidRevision]));
		});
	...
	describe('createRevision', () => {
		...
		it('should create a revision on an existing container', async () => {
			...
			await waitForActions(() => {
				...
				RevisionsActions.setUploadComplete(uploadId, false),
				ContainersActions.setContainerStatus(projectId, mockBody.containerId, UploadStatuses.QUEUED),
				RevisionsActions.setUploadComplete(uploadId, true),
			]);
