...
import { IFormRule, OPERATION_DISPLAY_NAMES } from '../../groupRulesForm.helpers';
import { ListSubheader } from '../groupRulesInputs.styles';
import { OPERATIONS_TYPES, Operator } from '@/v5/store/tickets/tickets.types';
import { useFormContext } from 'react-hook-form';

export const RuleOperator = ({ disabled }) => {
	const { setValue, getValues } = useFormContext<IFormRule>();
	const currentOperator = getValues('operator');
	const onOperatorChange = (e) => {
		const newOperator = e.target.value;
		const newOperatorType = OPERATIONS_TYPES[newOperator];
		if (newOperatorType !== OPERATIONS_TYPES[currentOperator]) {
			if (newOperatorType === 'numberRange') {
				setValue('values', [{ value: '' }, { value: '' }], { shouldDirty: true, shouldValidate: true });
			} else {
