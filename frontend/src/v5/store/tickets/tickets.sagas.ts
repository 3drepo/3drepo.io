...
export function* fetchTickets({ teamspace, projectId, modelId, isFederation, extraProperties }: FetchTicketsAction) {
	try {
		const fetchModelTickets = isFederation
			? API.Tickets.fetchFederationTickets
			: API.Tickets.fetchContainerTickets;
		const tickets = yield fetchModelTickets(teamspace, projectId, modelId, { extraProperties });
