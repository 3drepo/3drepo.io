...
const convertToV5GroupHierarchy = (group: any, type: ViewpointGroupHierarchyType): ViewpointGroupHierarchy => {
	...
	let name = '';
	switch (type) {
		case ViewpointGroupHierarchyType.COLORED:
			...
			break;
		case ViewpointGroupHierarchyType.TRANSFORMED:
...
export const getViewerState = async () => {
	...
	if (viewpointV4.override_groups?.length) {
		state.colored = viewpointV4.override_groups.map((group) => convertToV5GroupHierarchy(group, ViewpointGroupHierarchyType.COLORED));
	}
	...
	if (viewpointV4.transformation_groups) {
		state.transformed = viewpointV4.transformation_groups.map((group) => convertToV5GroupHierarchy(group, ViewpointGroupHierarchyType.TRANSFORMED));
	}
