...
def updateBackend(version):
    ...
    sedCmd("\"version\": \"[^ ]*\"", "\"version\": \"" + version +"\"", "backend/routes/apidoc.json");
    sedCmd("\"VERSION\" : \"[^ ]*\",", "\"VERSION\" : \"" + version +"\",", "backend/VERSION.json");
    execExitOnFail("git add backend/package.json backend/routes/apidoc.json backend/VERSION.json", "failed to add cmake to git")
